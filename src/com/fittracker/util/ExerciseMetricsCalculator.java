...
public class ExerciseMetricsCalculator {
    ...
    public static ExerciseMetrics calculateMetrics(Exercise exercise, WorkoutSet workoutSet, double userWeight) {
        ...
        // Calculate based on exercise category and available data
        switch (exercise.category.toUpperCase()) {
            case "STRENGTH" -> calculateStrengthMetrics(metrics, exercise, workoutSet, userWeight);
            case "CARDIO" -> calculateCardioMetrics(metrics, exercise, workoutSet, userWeight);
            case "FLEXIBILITY" -> calculateFlexibilityMetrics(metrics, exercise, workoutSet, userWeight);
            case "BALANCE" -> calculateBalanceMetrics(metrics, exercise, workoutSet, userWeight);
            default -> calculateGeneralMetrics(metrics, exercise, workoutSet, userWeight);
        }
    ...
    private static void calculateStrengthMetrics(ExerciseMetrics metrics, Exercise exercise, 
        ...
        if (set.weightKg != null && metrics.oneRepMax != null) {
            ...
            if (intensityPercentage >= 90) metrics.intensity = "Maximum (90%+ 1RM)";
            else if (intensityPercentage >= 80) metrics.intensity = "High (80-89% 1RM)";
            else if (intensityPercentage >= 65) metrics.intensity = "Moderate (65-79% 1RM)";
            else metrics.intensity = "Low (<65% 1RM)";
        }
    ...
    private static void calculateCardioMetrics(ExerciseMetrics metrics, Exercise exercise, 
        ...
        if (metValue >= 9) metrics.intensity = "Vigorous (>9 METs)";
        else if (metValue >= 6) metrics.intensity = "Moderate-Vigorous (6-9 METs)";
        else if (metValue >= 3) metrics.intensity = "Moderate (3-6 METs)";
        else metrics.intensity = "Light (<3 METs)";
    ...
    private static double getDefaultStrengthMET(String exerciseName) {
        ...
        if (name.contains("deadlift") || name.contains("squat")) return 6.0;
        if (name.contains("bench") || name.contains("press")) return 5.0;
        if (name.contains("curl") || name.contains("extension")) return 4.5;
        if (name.contains("pullup") || name.contains("chinup")) return 8.0;
        if (name.contains("pushup")) return 3.8;
        return 5.0; // Default strength training
    }
    
    private static double getDefaultCardioMET(String exerciseName) {
        String name = exerciseName.toLowerCase();
        if (name.contains("running") || name.contains("sprint")) return 9.8;
        if (name.contains("jogging")) return 7.0;
        if (name.contains("walking")) return 3.5;
        if (name.contains("cycling") || name.contains("bike")) return 6.8;
        if (name.contains("swimming")) return 8.0;
        if (name.contains("rowing")) return 7.0;
        if (name.contains("elliptical")) return 5.0;
        if (name.contains("stairs") || name.contains("climbing")) return 8.5;
        return 6.0; // Default cardio
    }
    
    private static String determineActivityType(String exerciseName) {
        String name = exerciseName.toLowerCase();
        if (name.contains("run") || name.contains("sprint")) return "RUNNING";
