...
public class WorkoutDAOImpl implements WorkoutDAO {
    ...
    public long createSession(WorkoutSession s) throws SQLException {
        try (Connection c = Database.getConnection();
            ...
            if (s.durationMin != null) cs.setInt(3, s.durationMin); else cs.setNull(3, Types.INTEGER);
    ...
    public void addSet(WorkoutSet w) throws SQLException {
        try (Connection c = Database.getConnection();
            ...
            cs.setInt(3, w.setNo);
            if (w.reps != null) cs.setInt(4, w.reps); else cs.setNull(4, Types.INTEGER);
    ...
    public boolean updateSessionDuration(long sessionId, int duration) throws SQLException {
        try (Connection c = Database.getConnection();
             PreparedStatement ps = c.prepareStatement("UPDATE WORKOUT_SESSION SET DURATION_MIN=? WHERE SESSION_ID=?")) {
    ...
    public StepMetrics calculateStepMetrics(int steps, String activityType, double userWeight, int durationMin) throws SQLException {
        try (Connection c = Database.getConnection();
            ...
            cs.registerOutParameter(1, Types.REF_CURSOR); // Oracle REF CURSOR
            cs.setInt(2, steps);
