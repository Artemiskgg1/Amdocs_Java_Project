...
public class UserDAOImpl implements UserDAO {
    ...
    public long create(User u) throws SQLException {
        String sql = "INSERT INTO USERS (FULL_NAME, EMAIL, GENDER, DOB, HEIGHT_CM) "
                + "VALUES (?, ?, ?, ?, ?)";
        try (Connection c = Database.getConnection(); PreparedStatement ps = c.prepareStatement(sql, new String[]{"USER_ID"})) {
    ...
    public boolean delete(long id) throws SQLException {
        try (Connection c = Database.getConnection(); PreparedStatement ps = c.prepareStatement("DELETE FROM USERS WHERE USER_ID=?")) {
            ps.setLong(1, id);
            return ps.executeUpdate() == 1;
        }
    }
