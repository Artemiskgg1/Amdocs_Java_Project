...
public class NutritionDAOImpl implements NutritionDAO {
    ...
    public long create(NutritionLog n) throws SQLException {
        String sql = "INSERT INTO NUTRITION_LOG (USER_ID, LOG_DATE, MEAL_TYPE, ITEM, CALORIES, PROTEIN_G, CARBS_G, FAT_G) "
                + "VALUES (?,?,?,?,?,?,?,?)";
        try (Connection c = Database.getConnection(); PreparedStatement ps = c.prepareStatement(sql, new String[]{"LOG_ID"})) {
            ps.setLong(1, n.userId);
            ps.setDate(2, n.logDate);
            ps.setString(3, n.mealType);
            ps.setString(4, n.item);
            if (n.calories != null) {
                ps.setInt(5, n.calories);
            } else {
                ps.setNull(5, Types.INTEGER);
            }
            if (n.proteinG != null) {
                ps.setDouble(6, n.proteinG);
            } else {
                ps.setNull(6, Types.NUMERIC);
            }
            if (n.carbsG != null) {
                ps.setDouble(7, n.carbsG);
            } else {
                ps.setNull(7, Types.NUMERIC);
            }
            if (n.fatG != null) {
                ps.setDouble(8, n.fatG);
            } else {
        ...
        try (Connection c = Database.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    n.proteinG = rs.getDouble(7);
                    if (rs.wasNull()) {
                        n.proteinG = null;
                    }
                    n.carbsG = rs.getDouble(8);
                    if (rs.wasNull()) {
                        n.carbsG = null;
                    }
                    n.fatG = rs.getDouble(9);
                    if (rs.wasNull()) {
                        n.fatG = null;
                    }
                    list.add(n);
